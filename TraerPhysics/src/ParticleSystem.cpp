...
namespace traer { namespace physics {
	...
	void ParticleSystem::setIntegrator(IntegratorType type) {
		delete integrator;
		if(type == IntegratorType::MODIFIED_EULER)
			integrator = new ModifiedEulerIntegrator(this);
		else if(type == IntegratorType::RUNGE_KUTTA)
    ...
    void ParticleSystem::clear()
    {
        ...
        for (size_t i = 0; i < customForces.size(); i++) {
    ...
    {
        integrator = new RungeKuttaIntegrator( this );
        gravity=Vec3f( 0, DEFAULT_GRAVITY, 0 );
        drag = DEFAULT_DRAG;
    }
